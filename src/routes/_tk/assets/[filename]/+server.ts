 ...
 *
 * GET /_tk/assets/{filename} - Get asset file
 *
 * Project resolution:
 *   - If ?project_id is provided, use that (for builder)
 *   - Otherwise, infer from request domain (for production apps)
 *
 * Built-in placeholders (no project needed):
 ...
 *
 * Query params:
 *   - project_id: Explicit project ID (optional, for builder)
...
export const GET: RequestHandler = async ({ params, url, request }) => {
	...
	// Handle built-in placeholders (no project needed)
